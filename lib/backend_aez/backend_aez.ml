...
let rec declare_stream name ty =
  ...
  | _ ->
      ...
      (try
       ...
       with Aez.Smt.(Error (DuplicateTypeName _)) -> ());
      Leaf h
...
let rec instantiate_node ?(inst_cnt = ref 0) ?(inst_name = "") nodes time_expr
  ...
  let rec convert_formula = function
    | Term (Bool true) -> Leaf Formula.f_true
    | Term (Bool false) -> Leaf Formula.f_false
    | Term (Var (id, _, time)) ->
  ...
  and convert_term = function
    | N -> Leaf time_expr (* Leaf (Term.make_app time []) *)
    | Bool true -> Leaf Term.t_true
    | Bool false -> Leaf Term.t_false
    | Int i -> Leaf (Term.make_int (Num.num_of_int i))
    | Float f -> Leaf (Term.make_real (num_of_float f))
    | Var (id, _, time) ->
        let var_tree = get_stream id in
        let time_expr = try_leaf @@ convert_term time in
    ...
    | Function (id, time_expr, args) -> (
        ...
        match outputs with
        | [] -> assert false
        | [ (_, o) ] -> tree_map (fun o -> Term.make_app o [ function_time ]) o
        | _ ->
            tree_map
