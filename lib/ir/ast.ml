...
let rec print_term fmt =
  ...
  | N ->
      fprintf fmt "N"
  | Bool b ->
      fprintf fmt "%a" pp_print_bool b
  | Int i ->
      fprintf fmt "%a" pp_print_int i
  | Float f ->
      fprintf fmt "%f" f
  | Var (id, name, t) ->
      fprintf fmt "var(%d, %s, %a)" id name print_term t
  | TupleTerm l ->
      fprintf
        fmt
        "(@[<h>%a@])"
  ...
  | Add ts ->
      fprintf
        ...
        ts
  | Sub (x, y) ->
      fprintf fmt "(%a - %a)" print_term x print_term y
  | Neg t ->
  ...
  | IfThenElse (cond, t1, t2) ->
      fprintf
        ...
        t2
  | IntOfReal t ->
      fprintf fmt "int(%a)" print_term t
  | RealOfInt t ->
      fprintf fmt "real(%a)" print_term t
...
let find_node ctx name =
  try Hashstr.find ctx.tbl name with
  | Not_found ->
...
let find_stream lctx name =
  try fst @@ Hashstr.find lctx.tbl name with
  | Not_found ->
...
let rec convert_to_term time_expr ctx local_ctx (expr : t_expr) =
  ...
  | TE_const (Creal f) ->
      Float f
  | TE_ident id ->
      Var (find_stream local_ctx id.name, id.name, time_expr)
  | TE_app (id, exprs) ->
  ...
  | TE_pre expr ->
      convert_to_term (Sub (time_expr, Int 1)) ctx local_ctx expr
  | TE_tuple exprs ->
      TupleTerm (List.map convert exprs)
  | TE_op ((Op_add | Op_add_f), exprs) ->
      Add (List.map convert exprs)
  | TE_op ((Op_sub | Op_sub_f), [ e ]) ->
      Neg (convert e)
  | TE_op ((Op_sub | Op_sub_f), [ e1; e2 ]) ->
      Sub (convert e1, convert e2)
  | TE_op ((Op_mul | Op_mul_f), exprs) ->
      Mul (List.map convert exprs)
  | TE_op ((Op_div | Op_div_f), [ e1; e2 ]) ->
      Div (convert e1, convert e2)
  | TE_op (Op_mod, [ e1; e2 ]) ->
      Mod (convert e1, convert e2)
...
and convert_to_formula time_expr ctx local_ctx (expr : t_expr) =
  ...
  | TE_pre expr ->
      Term (to_term ~t:(Sub (time_expr, Int 1)) expr)
  | TE_op (Op_if, [ pred; e1; e2 ]) ->
      Term (IfThenElse (to_form pred, to_term e1, to_term e2))
  | TE_op (Op_eq, exprs) ->
      Equal (List.map to_term exprs)
  | TE_op (Op_neq, exprs) ->
      Not (Equal (List.map to_term exprs))
  | TE_op (Op_lt, [ e1; e2 ]) ->
      Less (to_term e1, to_term e2)
  | TE_op (Op_le, [ e1; e2 ]) ->
      LessOrEqual (to_term e1, to_term e2)
  | TE_op (Op_gt, [ e1; e2 ]) ->
      Greater (to_term e1, to_term e2)
  | TE_op (Op_ge, [ e1; e2 ]) ->
      GreaterOrEqual (to_term e1, to_term e2)
  | TE_op (Op_not, [ e ]) ->
      Not (to_form e)
  | TE_op (Op_and, exprs) ->
      And (List.map to_form exprs)
  | TE_op (Op_or, exprs) ->
      Or (List.map to_form exprs)
  | TE_op (Op_impl, [ e1; e2 ]) ->
      Imply (to_form e1, to_form e2)
...
let convert_patt local_ctx { tpatt_desc; _ } =
  ...
  | [ x ] ->
      Var (find_stream local_ctx x.name, x.name, N)
  | l ->
      TupleTerm
        (List.map
