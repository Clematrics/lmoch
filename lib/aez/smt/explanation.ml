...
let iter_atoms f s =
  S.iter (fun e -> match e with Fresh _ -> () | Atom a -> f a) s

let fold_atoms f s acc =
  S.fold (fun e acc -> match e with Fresh _ -> acc | Atom a -> f a acc) s acc
...
let remove_fresh i s =
  let fi = Fresh i in
  if S.mem fi s then Some (S.remove fi s) else None
