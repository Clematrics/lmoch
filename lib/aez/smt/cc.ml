...
module type S = sig
  ...
  val assume :
    cs:bool -> Literal.LT.t -> Explanation.t -> t -> t * Term.Set.t * int
...
module Make (X : Sig.X) = struct
  ...
  type env = { use : Use.t; uf : Uf.t; relation : X.Rel.t }

  type choice_sign =
    | CPos of int
    (* The explication of this choice *)
    | CNeg
  (* The choice has been already negated *)
  ...
  let equal_only_by_congruence env ex t1 t2 acc =
    if T.equal t1 t2 then acc
      ...
      let { T.f = f1; xs = xs1; ty = ty1; _ } = T.view t1 in
      if X.fully_interpreted f1 then acc
      else
        let { T.f = f2; xs = xs2; ty = ty2; _ } = T.view t2 in
  ...
  let term_canonical_view env a ex_a = view (Uf.find env.uf) (A.LT.view a) ex_a
        ...
        | { T.f = f1; xs = [ x ]; _ } ->
            List.fold_left
              (fun acc t2 ->
                ...
                | { T.f = f2; xs = [ y ]; _ } when S.equal f1 f2 -> (
  ...
  let clean_use =
    List.fold_left (fun env (a, ex) ->
        match a with
        | LSem _ -> assert false
        | LTerm t -> (
            match A.LT.view t with
            | A.Distinct (_, lt) | A.Builtin (_, _, lt) ->
  ...
  let congruence_closure env r1 r2 ex =
    Print.cc r1 r2;
    ...
    List.fold_left
      (fun (env, l) (p, touched, v) ->
        ...
        let new_eqs =
          SetT.fold (fun t l -> congruents env t st_others l ex) p_t l
        in
        let touched_atoms =
          List.map (fun (x, y, e) -> (LSem (A.Eq (x, y)), e)) touched
        in
        let touched_atoms =
          SetA.fold (fun (a, ex) acc -> (LTerm a, ex) :: acc) p_a touched_atoms
        in
        let touched_atoms =
          SetA.fold
            (fun (a, ex) acc -> (LTerm a, ex) :: acc)
            sa_others touched_atoms
        in
  ...
  and semantic_view env choices la =
    List.fold_left
      (fun (env, choices, lsa) (a, ex) ->
        ...
        | LSem sa -> (env, choices, (sa, None, ex) :: lsa))
      (env, choices, []) la
        ...
        List.fold_left
          (fun (env, choices) (sa, _, ex) ->
            ...
            | A.Eq (r1, r2) ->
                ...
                else ({ env with uf = fst (Uf.union env.uf r1 r2 ex) }, choices)
            | A.Distinct (false, lr) ->
                if Uf.already_distinct env.uf lr then (env, choices)
                else ({ env with uf = Uf.distinct env.uf lr ex }, choices)
    ...
    let rec aux ch bad_last dl base_env li =
      ...
      | [], _ -> (
          ...
          | l ->
              ...
              let sz =
                List.fold_left
                  (fun acc (a, s, _, _) -> Num.mult_num acc s)
                  (Num.Int 1) (l @ dl)
              in
              Print.split_size sz;
      ...
      | ((c, size, CPos exp, ex_c_exp) as a) :: l, _ -> (
          try
            ...
            let base_env, ch =
              assume_literal base_env ch [ (LSem c, ex_c_exp) ]
            in
            aux ch bad_last (a :: dl) base_env l
    ...
    let r =
      try
        if t.choices = [] then look_for_sat [] t t.gamma []
          ...
          with Exception.Inconsistent dep ->
            look_for_sat ~bad_last:(Yes dep) [] { t with choices = [] } t.gamma
              t.choices
  ...
  let extract_terms_from_assumed =
    List.fold_left (fun acc (a, _) ->
        ...
        | LTerm r -> (
            ...
            | Literal.Eq (t1, t2) -> SetT.add t1 (SetT.add t2 acc)
  ...
  let add_and_process a t =
    let aux a ex env =
      let gamma, l = add env [] a ex in
      assume_literal gamma [] l
    in
  ...
  let empty () =
    let env = { use = Use.empty; uf = Uf.empty; relation = X.Rel.empty () } in
