...
module Make (Dummy : sig end) = struct
  module Solver_types = Solver_types.Make ()
  open Solver_types

  type state = { env : env; st_cpt_mk_var : int; st_ma : var Literal.LT.Map.t }
  ...
  let insert_var_order v = Iheap.insert f_weight env.order v.vid
  ...
  *)
  ...
  let progress_estimate () =
    ...
    for i = 0 to lvl do
      ...
      let _end =
        if i = lvl then Vec.size env.trail else Vec.get env.trail_lim i
      in
  ...
  let propagate_in_clause a c i watched new_sz =
    ...
    if first == a.neg then (
      (* le litiral faux doit etre dans .(1) *)
  ...
  let theory_propagate () =
    ...
    try
      ...
      env.tenv <-
        List.fold_left
          (fun t (a, ex) ->
            let t, _, _ = Th.assume ~cs:true a ex t in
            t)
          env.tenv !facts;
  ...
  let analyze c_clause =
    ...
    while !cond do
      ...
      for j = 0 to Vec.size !c.atoms - 1 do
        ...
        (* Pas sur *)
        if (not q.var.seen) && q.var.level > 0 then (
      ...
      while not (Vec.get env.trail !tr_ind).var.seen do
        decr tr_ind
      done;
      decr pathC;
  ...
  let f_sort_db c1 c2 =
    ...
    else if sz1 > 2 && (sz2 = 2 || c < 0) then -1
    else 1
...
*)
  ...
  let report_b_unsat ({ atoms; _ } as confl) =
    let l = ref [ confl ] in
    ...
    if false then (
      ...
      List.iter (fun hc -> eprintf "    %a@." Debug.clause hc) !l);
    ...
    if false then (
      eprintf "@.>>UNSAT_CORE:@.";
      List.iter (fun hc -> eprintf "    %a@." Debug.clause hc) unsat_core);
    env.is_unsat <- true;
  ...
  let simplify () =
    ...
    | None -> (
        ...
        | None -> ()));
    if nb_assigns () <> env.simpDB_assigns && env.simpDB_props <= 0 then (
  ...
  let theory_analyze dep =
    let atoms, sz, max_lvl, c_hist =
      Ex.fold_atoms
        ...
        dep ([], 0, 0, [])
    in
    if atoms = [] then (* check_inconsistence_of dep; *)
      report_t_unsat dep
    ...
    while true do
      ...
      | None -> (
          ...
          | None ->
              ...
              if
                n_of_learnts >= 0
                && Vec.size env.learnts - nb_assigns () >= n_of_learnts
              then reduce_db ();
  ...
  let solve () =
    ...
    let n_of_learnts =
      ref (to_float (nb_clauses ()) *. env.learntsize_factor)
    in
    try
      while true do
  ...
  let add_clauses cnf ~cnumber =
    ...
    match theory_propagate () with None -> () | Some dep -> report_t_unsat dep
  ...
  let restore { env = s_env; st_cpt_mk_var; st_ma } =
