...
module Make (X : OrderedType) : S with type elt = X.t = struct
  ...
  module V = struct
    ...
    let equal a1 a2 =
      ...
      | Distinct (b1, lt1), Distinct (b2, lt2) -> (
          try b1 = b2 && List.for_all2 (fun x y -> X.compare x y = 0) lt1 lt2
  ...
  let compare a1 a2 = compare a1.tag a2.tag
...
module LT : S_Term = struct
  ...
  let neg a =
    match view a with
    | Eq (t1, t2) when Term.equal t2 Term.faux -> make (Eq (t1, Term.vrai))
    | Eq (t1, t2) when Term.equal t2 Term.vrai -> make (Eq (t1, Term.faux))
    | _ -> L.neg a
