...
module H = struct
  ...
  let hash t =
    abs
      (List.fold_left
         (fun acc x -> (acc * 19) + x.tag)
         (Sy.hash t.f + Ty.hash t.ty)
         t.xs)

  let tag tag x = { x with tag }
end
...
let compare t1 t2 =
  let c = compare t2.tag t1.tag in
  if c = 0 then c
  else
...
let make s l ty = T.hashcons { f = s; xs = l; ty; tag = 0 (* dumb_value *) }
