...
module Make (X : ALIEN) = struct
  ...
  let is_mine_symb = function Sy.Name (_, Sy.Constructor) -> true | _ -> false
  let fully_interpreted _sb = true
  let type_info = function Cons (_, ty) -> ty | Alien x -> X.type_info x
  let is_mine_type c = match type_info c with Ty.Tsum _ -> true | _ -> false
  let color _ = assert false

  let print fmt = function
    | Cons (hs, _ty) -> fprintf fmt "%s" (Hs.view hs)
    | Alien x -> fprintf fmt "%a" X.print x

  let embed r = match X.extract r with Some c -> c | None -> Alien r
  let is_mine = function Alien r -> r | Cons (_hs, _ty) as c -> X.embed c
  ...
  let subst p v c =
    ...
    else match c with Cons (_hs, _t) -> cr | Alien r -> X.subst p v r

  let make t =
    match T.view t with
    | { T.f = Sy.Name (hs, Sy.Constructor); xs = []; ty; _ } ->
        (is_mine (Cons (hs, ty)), [])
    | _ -> assert false
  ...
  let solve a b =
    ...
    | Cons (_c1, _), Cons (_c2, _) -> raise Unsolvable
  ...
  module Rel = struct
    ...
    let add_diseq hss sm1 sm2 dep env eqs =
      ...
      | Alien r, Cons (h, ty) | Cons (h, ty), Alien r ->
          let enum, ex =
            try MX.find r env with Not_found -> (hss, Ex.empty)
          in
      ...
      | Alien _r1, Alien _r2 -> (env, eqs)
      | _ -> (env, eqs)

    let add_eq hss sm1 sm2 dep env eqs =
      match (sm1, sm2) with
      | Alien r, Cons (h, _ty) | Cons (h, _ty), Alien r ->
          let enum, ex =
            try MX.find r env with Not_found -> (hss, Ex.empty)
          in
          let ex = Ex.union ex dep in
      ...
      | Alien r1, Alien r2 ->
          let enum1, ex1 =
            try MX.find r1 env with Not_found -> (hss, Ex.empty)
          in
          let enum2, ex2 =
            try MX.find r2 env with Not_found -> (hss, Ex.empty)
          in
      ...
      let env, eqs =
        List.fold_left
          (fun (env, eqs) -> function
            | A.Eq (r1, r2), _, ex -> aux true r1 r2 ex env eqs (values_of r1)
    ...
    let add env r =
      ...
      | Alien r, Some hss ->
          if MX.mem r env then env else MX.add r (hss, Ex.empty) env
      | _ -> env
  end
end
