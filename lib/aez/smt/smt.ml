...
module Type = struct
  ...
  let declare_constructor ty c =
    ...
    H.add decl_symbs c (Symbols.name ~kind:Symbols.Constructor c, [], ty)
...
module Symbol = struct
  ...
  let declare f args ret =
    ...
    List.iter
      (fun t -> if not (H.mem decl_types t) then raise (Error (UnknownType t)))
      (ret :: args);
    H.add decl_symbs f (Symbols.name f, args, ret)

  let type_of s =
    let _, args, ret = H.find decl_symbs s in
    (args, ret)
  ...
  let not_builtin ty =
    ...
    || not
         (Hstring.equal ty Type.type_int
         || Hstring.equal ty Type.type_real
         || Hstring.equal ty Type.type_bool
         || Hstring.equal ty Type.type_proc)

  let has_abstract_type s =
    let _, ret = type_of s in
    match H.find decl_types ret with Ty.Tabstract _ -> true | _ -> false

  let has_type_proc s = Hstring.equal (snd (type_of s)) Type.type_proc
...
module Variant = struct
  ...
  let assign_var x y = if not (Hstring.equal x y) then add assignments x y
  ...
  let print () =
    H.iter
      (fun x c -> Format.eprintf "%a = {%a}@." Hstring.print x hset_print c)
      constructors
    ...
    List.iter
      (fun (x, nty) ->
        if not (H.mem constructors x) then
          ...
          match ty with
          | Ty.Tsum (_, l) -> H.add constructors x (set_of_list l)
          | _ -> ())
      l;
    H.clear assignments
  ...
  let update_decl_types s =
    ...
    HSet.iter
      (fun x ->
        ...
        nty := if !nty = "" then vx else !nty ^ "|" ^ vx)
      s;
...
and Formula : sig
  ...
  module Tseitin (Dymmy : sig end) : sig
    val make_cnf : t -> Literal.LT.t list list
  end
end = struct
  type comparator = Eq | Neq | Le | Lt
  ...
  let rec lift_ite env op l =
    try
      ...
      | Some true -> lift_ite (c :: env) op (left @ (t1 :: right))
      | Some false -> lift_ite (make Not [ c ] :: env) op (left @ (t2 :: right))
      | None ->
          Comb
            ( And,
              [
                Comb (Imp, [ c; lift_ite (c :: env) op (left @ (t1 :: right)) ]);
                Comb
                  ( Imp,
                    [
                      make Not [ c ];
    ...
    with Not_found ->
      let lit =
        match (op, l) with
        | Eq, [ Term.T t1; Term.T t2 ] -> Literal.Eq (t1, t2)
        | Neq, ts ->
            let ts =
              List.map (function Term.T x -> x | _ -> assert false) ts
            in
  ...
  let rec sform = function
    ...
    | Comb (And, l) -> Comb (And, List.map sform l)
    | Comb (Or, l) -> Comb (Or, List.map sform l)
    | Comb (Imp, [ f1; f2 ]) ->
        Comb (Or, [ sform (Comb (Not, [ f1 ])); sform f2 ])
    | Comb (Imp, _) -> assert false
    | f -> f

  let make_or = function [] -> assert false | [ a ] -> a | l -> Comb (Or, l)
  ...
  let rec cnf f =
    ...
    | Comb (Or, l) -> (
        ...
        let l_and, l_or =
          List.partition (function Comb (And, _) -> true | _ -> false) l
        in
        match l_and with
        | [ Comb (And, l_conj) ] ->
  ...
  let rec mk_cnf = function
    | Comb (And, l) -> List.fold_left (fun acc f -> mk_cnf f @@ acc) [] l
    | Comb (Or, [ f1; f2 ]) ->
        let ll1 = mk_cnf f1 in
        let ll2 = mk_cnf f2 in
        List.fold_left
          (fun acc l1 -> List.rev_map (fun l2 -> l1 @@ l2) ll2 @@ acc)
          [] ll1
    | Comb (Or, f1 :: l) ->
        let ll1 = mk_cnf f1 in
  ...
  let rec unfold mono f =
    ...
    | Comb (Or, l) -> List.fold_left unfold mono l
    | _ -> assert false

  let rec init monos f =
    match f with
    | Comb (And, l) -> List.fold_left init monos l
    | f -> unfold [] f :: monos
      ...
      let acc =
        List.fold_left
          (fun acc (p, l) ->
            proxies := p :: !proxies;
            let acc =
              List.fold_left (fun acc a -> [ p; Literal.LT.neg a ] :: acc) acc l
            in
...
module Make (Dummy : sig end) = struct
    ...
    List.iter
      (fun c ->
        eprintf "%a@." (Formula.print_list "or")
          (List.map (fun x -> Formula.Lit x) c))
      uc;
    eprintf "@.";
    try
      clear ();
  ...
  let export_unsatcore2 cl =
    let s =
      List.fold_left
        (fun s { Solver_types.name = n; _ } ->
          try SInt.add (int_of_string n) s with _ -> s)
        SInt.empty cl
    in
    SInt.elements s
  ...
  let assume ?(profiling = false) ~id f =
    ...
    try
      CSolver.assume (Tseitin.make_cnf f) ~cnumber:id;
