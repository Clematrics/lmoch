...
type borne = Strict of (num * Ex.t) | Large of (num * Ex.t) | Pinfty | Minfty
...
type t = { ints : (borne * borne) list; is_int : bool; expl : Ex.t }
...
let print_borne fmt = function
  ...
  | Strict (v, e) | Large (v, e) -> fprintf fmt "%s" (string_of_num v)
...
let print fmt { ints; is_int = b; expl = e } =
  List.iter (fun i -> fprintf fmt "%a" print_interval i) ints

let undefined ty =
  { ints = [ (Minfty, Pinfty) ]; is_int = ty = Ty.Tint; expl = Ex.empty }
...
let explain_borne = function Large (_, e) | Strict (_, e) -> e | _ -> Ex.empty
...
let check_one_interval b1 b2 is_int =
  ...
  | (Strict (v1, e1) | Large (v1, e1)), (Strict (v2, e2) | Large (v2, e2)) -> (
      let c = compare_num v1 v2 in
      if c > 0 then raise (EmptyInterval (Ex.union e2 e1));
      if c = 0 then
        match (b1, b2) with
...
let pos_borne b1 = compare_bornes b1 (borne_of true Ex.empty (Int 0)) >= 0
let pos_borne_strict b1 = compare_bornes b1 (borne_of true Ex.empty (Int 0)) > 0
let neg_borne b1 = compare_bornes b1 (borne_of true Ex.empty (Int 0)) <= 0
let neg_borne_strict b1 = compare_bornes b1 (borne_of true Ex.empty (Int 0)) < 0
let zero_borne b1 = compare_bornes b1 (borne_of true Ex.empty (Int 0)) = 0

exception Found of Sig.answer

let doesnt_contain_0 { ints = l; _ } =
  try
    let max =
      List.fold_left
        (fun old_u (l, u) ->
          ...
          if neg_borne_strict old_u && pos_borne_strict l then
            raise
              (Found
                 (Sig.Yes (Ex.union (explain_borne old_u) (explain_borne l))));
          u)
        Minfty l
    in
    if neg_borne_strict max then Sig.Yes (explain_borne max) else Sig.No
  with Found ans -> ans
...
let is_strict_smaller i1 i2 =
  ...
  | _ -> (
      try
        List.iter2
          (fun (l1, u1) (l2, u2) ->
            if compare_bornes l1 l2 > 0 || compare_bornes u1 u2 < 0 then
              raise Exit)
          i1 i2;
        false
      with
...
let is_strict_smaller { ints = i1; _ } { ints = i2; _ } =
...
let rec union_bornes l =
  ...
  | [] | [ _ ] -> l
  | (l1, u1) :: ((l2, u2) :: r as r2) ->
      if compare_bornes u1 l2 < 0 then (l1, u1) :: union_bornes r2
      else if compare_bornes u1 u2 > 0 then union_bornes ((l1, u1) :: r)
      else union_bornes ((l1, u2) :: r)

let union ({ ints = l; _ } as uints) =
...
let add_borne b1 b2 =
  ...
  | Minfty, _ | _, Minfty -> Minfty
  | Pinfty, _ | _, Pinfty -> Pinfty
  | Large (v1, e1), Large (v2, e2) -> Large (v1 +/ v2, Ex.union e1 e2)
...
let add { ints = l1; is_int; expl = e1 } { ints = l2; expl = e2; _ } =
  let l =
    List.fold_left
      (fun l bs ->
        let i = add_interval l1 bs in
        i @ l)
      [] l2
  in
  union { ints = l; is_int; expl = Ex.union e1 e2 }
...
let mult_borne b1 b2 =
  ...
  | Minfty, Pinfty | Pinfty, Minfty -> assert false
  | Minfty, b | b, Minfty ->
      if compare_bornes b (borne_of true Ex.empty (Int 0)) = 0 then b
  ...
  | Strict (v1, e1), Strict (v2, e2)
  | Strict (v1, e1), Large (v2, e2)
...
type interval_class = P of Ex.t | M of Ex.t | N of Ex.t | Z
...
let mult_bornes (a, b) (c, d) =
  ...
  | P e1, P e2 -> (mult_borne_inf a c, mult_borne_sup b d, Ex.union e1 e2)
  | P e1, M e2 -> (mult_borne_inf b c, mult_borne_sup b d, Ex.union e1 e2)
  | P e1, N e2 -> (mult_borne_inf b c, mult_borne_sup a d, Ex.union e1 e2)
  | M e1, P e2 -> (mult_borne_inf a d, mult_borne_sup b d, Ex.union e1 e2)
  | M e1, M e2 ->
      ( min_borne (mult_borne_inf a d) (mult_borne_inf b c),
        max_borne (mult_borne_sup a c) (mult_borne_sup b d),
        Ex.union e1 e2 )
  | M e1, N e2 -> (mult_borne_inf b c, mult_borne_sup a c, Ex.union e1 e2)
  | N e1, P e2 -> (mult_borne_inf a d, mult_borne_sup b c, Ex.union e1 e2)
  | N e1, M e2 -> (mult_borne_inf a d, mult_borne_sup a c, Ex.union e1 e2)
  | N e1, N e2 -> (mult_borne_inf b d, mult_borne_sup a c, Ex.union e1 e2)
  | Z, (P _ | M _ | N _ | Z) -> (a, b, Ex.empty)
  | (P _ | M _ | N _), Z -> (c, d, Ex.empty)

let rec power_borne_inf p b =
  match p with 1 -> b | p -> mult_borne_inf b (power_borne_inf (p - 1) b)

let rec power_borne_sup p b =
  match p with 1 -> b | p -> mult_borne_sup b (power_borne_sup (p - 1) b)
...
let int_div_bornes l u = (int_div_of_borne_inf l, int_div_of_borne_sup u)

let intersect ({ ints = l1; expl = e1; is_int } as uints1)
    { ints = l2; expl = e2; _ } =
    ...
    | (lo1, up1) :: r1, (lo2, up2) :: r2 ->
        ...
        if cul < 0 then
          ...
          | (lor2, upr2) :: rr2 ->
              ...
              step (l1, r2) acc expl
        else if cll = 0 && cuu = 0 then step (r1, r2) ((lo1, up1) :: acc) expl
        else if cll <= 0 && cuu >= 0 then step (l1, r2) ((lo2, up2) :: acc) expl
        else if cll >= 0 && cuu <= 0 then step (r1, l2) ((lo1, up1) :: acc) expl
  ...
  if l = [] then raise (NotConsistent expl) else { uints1 with ints = l; expl }
...
let complement ({ ints = l; expl = e; _ } as uints) =
  let rec step l prev acc =
    match l with
    | (b1, b2) :: r ->
        let bu =
          match b1 with Strict v -> Large v | Large v -> Strict v | _ -> b1
        in
        let bl =
          match b2 with Strict v -> Large v | Large v -> Strict v | _ -> b2
        in
        if bu = Minfty then step r bl acc else step r bl ((prev, bu) :: acc)
    | [] ->
        if prev = Pinfty then List.rev acc else List.rev ((prev, Pinfty) :: acc)
  in
  { uints with ints = step l Minfty [] }

let exclude uints1 uints2 = intersect (complement uints1) uints2
  ...
  union
    {
      ints = resl;
      is_int = u1.is_int;
      expl = Ex.union expl (Ex.union u1.expl u2.expl);
    }
...
let root_default_borne is_int x n =
  ...
  | Large (v, e) | Strict (v, e) ->
      let s =
        if v >=/ Int 0 then root_default_num v n
        else minus_num (root_exces_num (minus_num v) n)
      in
      if is_int then
        let cs = ceiling_num s in
        let cs2 = cs **/ Int n in
        if v <=/ cs2 then Large (cs, e) else Large (cs +/ Int 1, e)
      else Large (s, e)
...
let root_exces_borne is_int x n =
  ...
  | Large (v, e) | Strict (v, e) ->
      let s =
        if v >=/ Int 0 then root_exces_num v n
        else minus_num (root_default_num (minus_num v) n)
      in
      if is_int then
...
let sqrt_interval is_int (b1, b2) =
  let l1, u1 =
    ( minus_borne (root_exces_borne is_int b2 2),
      minus_borne (root_default_borne is_int b1 2) )
  in
  let l2, u2 = (root_default_borne is_int b1 2, root_exces_borne is_int b2 2) in
  if compare_bornes l1 u1 > 0 then
    if compare_bornes l2 u2 > 0 then [] else [ (l2, u2) ]
...
let sqrt { ints = l; is_int; expl = e } =
  let l = List.fold_left (fun l' bs -> sqrt_interval is_int bs @ l') [] l in
  union { ints = l; is_int; expl = e }

let rec root n ({ ints = l; is_int; expl = e } as u) =
  if n mod 2 = 0 then root (n / 2) (sqrt u)
  else
    let l = List.fold_left (fun l' bs -> root_interval is_int bs n @ l') [] l in
    union { ints = l; is_int; expl = e }

let finite_size { ints = l; is_int; _ } =
  ...
  try
    let l' =
      List.fold_left
        (fun acc (l, u) ->
          if
            (pos_borne_strict l && pos_borne_strict u)
            || (neg_borne_strict l && neg_borne_strict u)
          then inv_bornes (l, u) is_int :: acc
          else raise Exit)
        [] l
    in
    union { u with ints = l' }
...
let div i1 i2 =
  ...
  if inv_i2.ints = [ (Minfty, Pinfty) ] then inv_i2
    ...
    let i1 =
      match doesnt_contain_0 i2 with
      | Sig.Yes ex -> add_expl_zero i1 ex
      | Sig.No -> i1
    in
    let ({ ints = l; is_int; _ } as i) = mult i1 inv_i2 in
    let l =
      if is_int then List.map (fun (l, u) -> int_div_bornes l u) l else l
    in
    { i with ints = l }
