...
module Make (X : Sig.X) (P : EXTENDED_Polynome with type r = X.r) = struct
  ...
  module LR = Literal.Make (struct
    type t = X.r

    include X
  end)

  module Seq = Set.Make (struct
    type t = r L.view * L.LT.t option * Explanation.t

    let compare (a, _, _) (b, _, _) = LR.compare (LR.make a) (LR.make b)
  end)
  ...
  let replace_inequation env x ineq =
    { env with inequations = (x, ineq) :: List.remove_assoc x env.inequations }
  ...
  let mult_bornes_vars vars monomes ty =
    List.fold_left
      (fun ui (y, n) ->
        let ui' =
          try fst (MX.find y monomes) with Not_found -> Intervals.undefined ty
        in
        Intervals.mult ui (Intervals.power n ui'))
      (Intervals.point (Int 1) ty Explanation.empty)
      vars
  ...
  let intervals_from_monomes env p =
    ...
    List.fold_left
      ...
      (Intervals.point v (P.type_info p) Explanation.empty)
      pl
  ...
  let rec add_monome expl use_x env x =
    ...
    with Not_found -> update_monome expl use_x env x
  ...
  and init_alien expl p (normal_p, c, d) ty use_x env =
    ...
    let i =
      try
        ...
        let old_i =
          Intervals.scale d
            (Intervals.add old_i (Intervals.point c ty Explanation.empty))
        in
  ...
  and update_monome expl use_x env x =
    ...
    let ui, env =
      ...
      | Some t -> (
          ...
          | { Term.f = Sy.Op Sy.Div; xs = [ a; b ]; _ } ->
              ...
              let ia, ib =
                match Intervals.doesnt_contain_0 ib with
                | Yes ex when Num.compare_num ca cb = 0 && P.compare pa' pb' = 0
                  ->
    ...
    let u, use_x' =
      ...
      with Not_found -> (Intervals.undefined (X.type_info x), use_x)
    in
    ...
    List.fold_left
      (fun monomes (a, x) ->
        ...
        try
          ...
          let new_ix =
            Intervals.scale (Int 1 // a)
              (Intervals.add i
                 (Intervals.scale (minus_num d)
                    (Intervals.add inp (Intervals.point c ty Explanation.empty))))
          in
  ...
  let update_polynomes env expl =
    let polynomes, monomes, improved =
      MP.fold
        ...
        env.polynomes
        (env.polynomes, env.monomes, env.improved)
    in
    { env with polynomes; monomes; improved }

  let find_one_eq x u =
    match Intervals.is_point u with
    | Some (v, ex) when X.type_info x <> Ty.Tint || is_integer_num v ->
        let eq = L.Eq (x, P.alien_of (P.create [] v (X.type_info x))) in
        Some (eq, None, ex)
    | _ -> None

  let find_eq eqs x u env =
    match find_one_eq x u with None -> eqs | Some eq1 -> eq1 :: eqs
  ...
  let ineq_status ({ Inequation.ple0 = p; is_le; _ } as ineq) =
    ...
    | None ->
        if P.is_empty p then
          ...
          else if c = 0 && is_le then Trivial_eq
          else Trivial_ineq v
        else Other
  ...
  let fm_equalities env eqs { Inequation.ple0 = p; dep; expl = ex; _ } =
    let inqs, eqs =
      List.fold_left
  ...
  let update_intervals env eqs expl (a, x, v) is_le =
    ...
    let b = Int (-1) */ v // a in
    let u =
      if a >/ Int 0 then Intervals.new_borne_sup expl b ~is_le uints
      else Intervals.new_borne_inf expl b ~is_le uints
    in
  ...
  let update_ple0 env p0 is_le expl =
    if P.is_empty p0 then env
      ...
      let p, change =
        if a </ Int 0 then (P.mult (P.create [] (Int (-1)) ty) p0, true)
        else (p0, false)
      in
      ...
      let u =
        ...
        else Intervals.new_borne_sup expl c ~is_le (Intervals.undefined ty)
      in
      let u, pu =
        try
  ...
  let add_inequations acc lin expl =
    List.fold_left
      (fun (env, eqs) ineq ->
        ...
        | Bottom -> raise (Exception.Inconsistent expl)
        | Trivial_eq -> fm_equalities env eqs ineq
        | Trivial_ineq c ->
            let n, pp =
              List.fold_left
              ...
              (fun acc
                ...
                let p =
                  P.add
                    ...
                    (P.mult (P.create [] n1 (P.type_info p1)) p2)
                in
  ...
  let split x l =
    let split_rec (cp, cn, co) ineq =
      try
        let a = Inequation.find x ineq in
        if a >/ Int 0 then (ineq :: cp, cn, co) else (cp, ineq :: cn, co)
  ...
  let choose_var l =
    ...
    let xopt =
      MX.fold
        ...
        pos_neg None
    in
    match xopt with Some (x, _) -> x | None -> raise Not_found
...
*)
  ...
  let fm env eqs expl = fourier (env, eqs) (List.map snd env.inequations) expl
  ...
  let add_disequality env eqs p expl =
    ...
    | [], Int 0 -> raise (Exception.Inconsistent expl)
    | [], v -> (env, eqs)
    | [ (a, x) ], v ->
    ...
    | _ ->
        ...
        let p =
          if a >=/ Int 0 then p else P.mult (P.create [] (Int (-1)) ty) p
        in
        let p, c, _ = P.normal_form p in
        let i1 = Intervals.point (minus_num c) ty expl in
        let i2 =
          try MP.find p env.polynomes with Not_found -> Intervals.undefined ty
        in
        let i = Intervals.exclude i1 i2 in
        let env =
          if Intervals.is_strict_smaller i i2 then
            let polynomes = MP.add p i env.polynomes in
            let monomes = update_monomes_from_poly p i polynomes env.monomes in
            let improved = SP.add p env.improved in
            { env with polynomes; monomes; improved }
          else env
        in
  ...
  let add_equality env eqs p expl =
    ...
    match P.to_list p with
    | [], Int 0 -> (env, eqs)
    | [], v -> raise (Exception.Inconsistent expl)
    | [ (a, x) ], v ->
        let b = minus_num v // a in
    ...
    | _ ->
        ...
        in
        let env =
          { env with known_eqs = SX.add (P.alien_of p) env.known_eqs }
        in
        (env, eqs)
  ...
  let normal_form a =
    ...
    | L.Builtin (true, n, [ r1; r2 ])
      when (not (is_le n)) && X.type_info r1 = Ty.Tint ->
  ...
  let remove_trivial_eqs eqs la =
    let set_of l = List.fold_left (fun s e -> Seq.add e s) Seq.empty l in
  ...
  let assume env la =
    ...
    let env, eqs, new_ineqs, expl =
      List.fold_left
        (fun (env, eqs, new_ineqs, expl) (a, root, e) ->
          ...
          try
            ...
            | L.Builtin (_, n, [ r1; r2 ]) when is_le n || is_lt n ->
                let root =
                  match root with Some a -> a | None -> assert false
                in
                let p1 = P.poly_of r1 in
                let p2 = P.poly_of r2 in
                let ineq = Inequation.create p1 p2 (is_le n) root expl in
                let env = init_monomes env ineq.Inequation.ple0 SX.empty expl in
                let env = update_ple0 env ineq.Inequation.ple0 (is_le n) expl in
        ...
        (env, [], false, Explanation.empty)
        la
    in

    if new_ineqs then if false then ();
    try
      ...
      in
      (env, result)
    with Intervals.NotConsistent expl -> raise (Exception.Inconsistent expl)
  ...
  let extract_improved env =
    SP.fold
      (fun p acc -> MP.add p (MP.find p env.polynomes) acc)
