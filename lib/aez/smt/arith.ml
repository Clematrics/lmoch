...
struct
  ...
  let is_mine_symb = function
    | Sy.Int _ | Sy.Real _
    | Sy.Op (Sy.Plus | Sy.Minus | Sy.Mult | Sy.Div | Sy.Modulo) ->
        true
    | _ -> false
  ...
  let rec mke coef p t ctx =
    let { T.f = sb; xs; ty; _ } = T.view t in
    match (sb, xs) with
...
*)
  ...
  let mod_sym a b =
    ...
    let m =
      if m </ Int 0 then if m >=/ minus_num b then m +/ b else assert false
      else if m <=/ b then m
      else assert false
    in
  ...
  let mult_const p c = P.mult p (P.create [] c (P.type_info p))
  ...
  let extract_min = function
    ...
    | (a, x) :: s ->
        List.fold_left
          (fun ((a, x), l) (b, y) ->
            if abs_num a <=/ abs_num b then ((a, x), (b, y) :: l)
            else ((b, y), (a, x) :: l))
          ((a, x), [])
          s
    ...
    | _ ->
        ...
        let a, l, b =
          if compare_num a (Int 0) < 0 then
            ( minus_num a,
              List.map (fun (a, x) -> (minus_num a, x)) l,
              minus_num b )
          else (a, l, b)
        in
  ...
  let solve_real p =
    try
      ...
      let p =
        P.mult (P.create [] (Int (-1) // a) (P.type_info p)) (P.remove x p)
      in
      [ (x, is_mine p) ]
