...
module Make (R : Sig.X) = struct
  ...
  module Lit = Literal.Make (struct
    type t = R.r
  ...
  module MapR = Map.Make (struct
    type t = R.r

    let compare = R.compare
  end)

  module SetR = Set.Make (struct
    type t = R.r
  ...
  module Env = struct
      ...
      let mapl =
        MapL.fold
          (fun l1 ex1 mapl ->
            ...
            with Not_found -> MapL.add l1 (Ex.union ex1 dep) mapl)
          nq_r1 nq_r2
      in
    ...
    let init_leaf env p =
      ...
      {
        env with
        repr = (if in_repr then env.repr else MapR.add p (p, Ex.empty) env.repr);
        classes =
          (if in_repr then env.classes else update_classes p p env.classes);
        gamma = (if in_repr then env.gamma else add_to_gamma p p env.gamma);
        neqs = (if in_neqs then env.neqs else update_neqs p p Ex.empty env);
      }
    ...
    let init_term env t =
      ...
      ( {
          ...
          neqs =
            ...
            else MapR.add rp MapL.empty env.neqs);
        },
        ctx )
  ...
  let x_solve env r1 r2 dep =
    ...
    if R.equal rr1 rr2 then [] (* Remove rule *)
      ...
      try R.solve rr1 rr2 with Unsolvable -> raise (Inconsistent dep))
      ...
      List.fold_left
        (fun (env, mapr, newds) r ->
          ...
          with Not_found ->
            ...
            let mdis =
              try MapR.find rr env.neqs with Not_found -> MapL.empty
            in
            let mdis =
      ...
      (fun env (r1, ex1, mapr) ->
        MapR.fold
          (fun r2 ex2 env ->
            ...
            try
              match R.solve r1 r2 with
              | [ (a, b) ] ->
                  if
                    (R.equal a r1 && R.equal b r2)
                    || (R.equal a r2 && R.equal b r1)
                  then env
