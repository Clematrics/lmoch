...
module rec CX : sig
  ...
  type r = Term of Term.t | X1 of X1.t | X5 of X5.t
  ...
  and compare_tag a b = Stdlib.compare (theory_num a) (theory_num b)
  ...
  let leaves r =
    match r with X1 t -> X1.leaves t | X5 t -> X5.leaves t | Term _ -> [ r ]
  ...
  let make t =
    let { Term.f = sb; _ } = Term.view t in
  ...
  let rec solve_list solved l =
    List.fold_left
      (fun solved (a, b) ->
        ...
        if cmp = 0 then solved
        else
          match (a, b) with
          (* both sides are empty *)
          | Term _, Term _ -> add_mr solved (unsolvable_values cmp a b)
          (* only one side is empty *)
          | a, b when unsolvable a || unsolvable b || compare_tag a b = 0 ->
  ...
  and unsolvable_values cmp a b =
    ...
    | X1 _, X5 _ | X5 _, X1 _ -> assert false
  ...
  let type_info = function
    ...
    | X5 t -> X5.type_info t
    | Term t ->
        let { Term.ty; _ } = Term.view t in
        ty

  module Rel = struct
    type elt = r
    type r = elt
    type t = { r1 : X1.Rel.t; r5 : X5.Rel.t }

    let empty _ = { r1 = X1.Rel.empty (); r5 = X5.Rel.empty () }

    let assume env sa =
      let env1, { assume = a1; remove = rm1 } = X1.Rel.assume env.r1 sa in
      let env5, { assume = a5; remove = rm5 } = X5.Rel.assume env.r5 sa in
      ({ r1 = env1; r5 = env5 }, { assume = a1 @ a5; remove = rm1 @ rm5 })
    ...
    let add env r = { r1 = X1.Rel.add env.r1 r; r5 = X5.Rel.add env.r5 r }
  end
end
