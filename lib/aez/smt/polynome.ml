...
module Make (X : S) = struct
  ...
  let pprint fmt p =
    M.iter
      ...
      p.m;
    let s, n =
      if p.c >=/ Int 0 then ("+", string_of_num p.c)
      else ("-", string_of_num (minus_num p.c))
    in
    fprintf fmt "%s%s" s n

  let print fmt p =
    M.iter (fun t n -> fprintf fmt "%s*%a " (string_of_num n) X.print t) p.m;
  ...
  let create l c ty =
    let m =
      List.fold_left
        (fun m (n, x) ->
          let n' = n +/ find x m in
          if n' =/ Int 0 then M.remove x m else M.add x n' m)
        M.empty l
    in
    { m; c; ty }
  ...
  let mult_monome a x p =
    ...
    let m = M.fold (fun xi ai m -> M.add (X.mult x xi) (a */ ai) m) p.m acx.m in
    { acx with m }
  ...
  let sub p1 p2 = add p1 (mult (create [] (Int (-1)) p1.ty) p2)
  ...
  let modulo p1 p2 =
    if M.is_empty p2.m then
      if p2.c =/ Int 0 then raise Division_by_zero
      else if M.is_empty p1.m then { p1 with c = mod_num p1.c p2.c }
  ...
  let abs_big_int_to_num b =
    let b = try Int (Big_int.int_of_big_int b) with Failure _ -> Big_int b in
    abs_num b

  let ppmc_denominators { m; _ } =
    let res =
      M.fold (fun k c acc -> ppmc_bi (denominator c) acc) m Big_int.unit_big_int
    in
    abs_num (num_of_big_int res)

  let pgcd_numerators { m; _ } =
    let res =
      M.fold (fun k c acc -> pgcd_bi (numerator c) acc) m Big_int.zero_big_int
    in
    abs_num (num_of_big_int res)

  let normal_form ({ m; c; _ } as p) =
    if M.is_empty m then ({ p with c = Int 0 }, p.c, Int 1)
    else
