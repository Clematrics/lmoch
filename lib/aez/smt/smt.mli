...
module Type : sig
  ...
  (** - [declare n cstrs] declares a new enumerated data-type with name [n] and
        constructors [cstrs].
  ...
  (** [constructors ty] returns the list of constructors of [ty] when type is an
      enumerated data-type, otherwise returns [\[\]].*)
end
...
module Symbol : sig
  ...
  (** [declare s \[arg_1; ... ; arg_n\] out] declares a new function symbol with
      type [ (arg_1, ... , arg_n) -> out] *)
...
(** {3 Variants}
    ...
    The types of symbols (when they are enumerated data types) can be refined to
    substypes of their original type (i.e. a subset of their constructors). *)
module Variant : sig
  val init : (Symbol.t * Type.t) list -> unit
  (** [init l] where [l] is a list of pairs [(s, ty)] initializes the type (and
      associated constructors) of each [s] to its original type [ty].

      This function must be called with a list of all symbols before attempting
      to refine the types. *)
  ...
  (** [assign_constr s cstr] will add the constraint that the constructor [cstr]
      must be in the type of [s] *)

  val assign_var : Symbol.t -> Symbol.t -> unit
  (** [assign_var x y] will add the constraint that the type of [y] is a subtype
      of [x] (use this function when [x := y] appear in your program *)
  ...
  (** [get_variants s] returns a set of constructors, which is the refined type
      of [s].*)
end
...
module rec Term : sig
  ...
  (** [make_app f l] creates the application of function symbol [f] to a list of
      terms [l]. *)
...
and Formula : sig
  ...
  type t = Lit of Literal.LT.t | Comb of combinator * t list
...
module type Solver = sig
  ...
  *)
  ...
  (** [assume ~profiling:b f id] adds the formula [f] to the context of the
      solver with idetifier [id]. This function only performs unit propagation.

      @param profiling
        if set to [true] then profiling information (time) will be computed
        (expensive because of system calls).

        {b Raises} {!Unsat} if the context becomes inconsistent after unit
        propagation. *)

  val check : ?profiling:bool -> unit -> unit
  (** [check ()] runs Alt-Ergo Zero on its context. If [()] is returned then the
      context is satifiable.

      @param profiling
        if set to [true] then profiling information (time) will be computed
        (expensive because of system calls).

        {b Raises} {!Unsat} [\[id_1; ...; id_n\]] if the context is
        unsatisfiable. [id_1, ..., id_n] is the unsat core (returned as the
        identifiers of the formulas given to the solver). *)
  ...
  (** [entails ~id f] returns [true] if the context of the solver entails the
      formula [f]. It doesn't modify the context of the solver (the state is
      saved when this function is called and restored on exit).*)
end
