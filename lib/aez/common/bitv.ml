...
let _ =
  for i = 1 to bpi do
    low_mask.(i) <- low_mask.(i - 1) lor bit_j.(pred i)
  done
  ...
  if j == 0 then
    ...
    if d > 0 then (
      ...
      Array.unsafe_set v (succ i')
        ...
        lor keep_highest_bits (Array.unsafe_get v (succ i')) (bpi - d)))
    else
      Array.unsafe_set v i'
...
let blit_int a v n =
  ...
  if j == 0 then Array.unsafe_set v i a
    ...
    Array.unsafe_set v (succ i)
      (keep_highest_bits (Array.unsafe_get v (succ i)) (bpi - j)
      lor (a lsr (bpi - j))))
...
let unsafe_blit v1 ofs1 v2 ofs2 len =
  if len > 0 then
    ...
    if bi == ei then blit_bits (Array.unsafe_get v1 bi) bj len v2 ofs2
    else (
...
let append v1 v2 =
  let l1 = v1.length and l2 = v2.length in
...
let blit_zeros v ofs len =
  if len > 0 then
    ...
    if bi == ei then blit_bits 0 bj len v ofs
    else (
...
let blit_ones v ofs len =
  if len > 0 then
    let bi, bj = pos ofs in
    let ei, ej = pos (ofs + len - 1) in
    if bi == ei then blit_bits max_int bj len v ofs
    else (
...
let all_zeros v =
  let b = v.bits in
  let n = Array.length b in
  let rec test i = i == n || (Array.unsafe_get b i == 0 && test (succ i)) in
  test 0
...
let all_ones v =
  ...
  let rec test i =
    ...
    else Array.unsafe_get b i == max_int && test (succ i)
  in
  test 0
...
struct
  ...
  let of_string s =
    ...
    for i = 0 to n - 1 do
      ...
      if c = '1' then unsafe_set v (if I.least_first then i else n - 1 - i) true
      else if c <> '0' then invalid_arg "Bitv.of_string"
    done;
    v
end

module L = S (struct
  let least_first = true
end)

module M = S (struct
  let least_first = false
end)
...
let output_bin out_ch v =
  ...
  let rec loop i pow byte =
    ...
    if i = len - 1 then output_byte out_ch byte
    else if i mod 8 = 7 then (
      output_byte out_ch byte;
      loop (i + 1) 1 0)
    else loop (i + 1) (pow * 2) byte
  in
...
(* [int] *)
let of_int_us i = { length = bpi; bits = [| i land max_int |] }
...
let to_int32_us v =
  ...
  | 32 ->
      Int32.logor
        (Int32.of_int v.bits.(0))
        (Int32.shift_left (Int32.of_int (v.bits.(1) land 1)) 30)
  | 64 -> Int32.of_int (v.bits.(0) land 0x7fffffff)
  | _ -> assert false
...
let to_int32_s v =
  ...
  | 64 -> Int32.of_int (v.bits.(0) land ffffffff)
  | _ -> assert false
...
let to_int64_us v =
  ...
  | 32 ->
      Int64.logor
        (Int64.of_int v.bits.(0))
        (Int64.logor
           (Int64.shift_left (Int64.of_int v.bits.(1)) 30)
