...
let report fmt = function
  ...
  | ExpectedType (t1, t2) ->
      fprintf fmt "this expression has type %a but is expected to have type %a"
        print_type t1 print_type t2
  | ExpectedPattern ty ->
      fprintf fmt "this pattern is expected to have type %a" print_type ty
  | ExpectedBase ty ->
      fprintf fmt
  ...
  | UndefinedOutputs l ->
      fprintf fmt "those output variables are undefined:%a"
        (fun fmt -> List.iter (fun x -> fprintf fmt "%s " x))
        l
  | InputVar s -> fprintf fmt "%s is an input variable" s
  | Causality -> fprintf fmt "problem of causality"
  | BadMain (t_in, t_out) ->
      fprintf fmt
        "The main node has type %a -> %a but is expected to have type %a -> \
         bool"
        print_type t_in print_type t_out print_type t_in
...
module Delta = struct
  ...
  let find n =
    try (Hashtbl.find nodes n, false)
    with Not_found -> (List.assoc n prims, true)
...
module Gamma = struct
  ...
  let adds loc io = List.fold_left (fun env (x, t) -> add loc env x t io)

  let find loc env x =
    try M.find x env with Not_found -> error loc (UnboundVar x)

  let patts_vars env =
    M.fold
      (fun _ (x, _, io) s -> if io = Vpatt then S.add x s else s)
      env S.empty
end

let base_ty_of_ty loc t =
  match t with [ t' ] -> t' | _ -> error loc (ExpectedBase t)

let compatible_base actual_ty expected_ty = actual_ty = expected_ty
...
let not_a_nested_tuple e loc =
  ...
  | PE_tuple el ->
      List.iter
        (fun e ->
          match e.pexpr_desc with PE_tuple _ -> error loc FlatTuple | _ -> ())
        el
  | _ -> assert false
...
and type_expr_desc env loc = function
  ...
  | PE_op
      ( ( Op_eq | Op_neq | Op_lt | Op_le | Op_gt | Op_ge | Op_add | Op_sub
        | Op_mul | Op_div | Op_mod | Op_add_f | Op_sub_f | Op_mul_f | Op_div_f
        | Op_not | Op_and | Op_or | Op_impl | Op_if ),
        [] ) ->
      error loc TooFewArguments
  | PE_op
      ( ( Op_eq | Op_neq | Op_lt | Op_le | Op_gt | Op_ge | Op_add | Op_mul
        | Op_div | Op_mod | Op_add_f | Op_mul_f | Op_div_f | Op_and | Op_or
        | Op_impl | Op_if ),
        [ _ ] ) ->
      error loc TooFewArguments
  ...
  | PE_app (f, el) -> (
      try
        ...
        (app_node, match t_out with [] -> assert false | _ -> t_out)
  ...
  | PE_arrow (e1, e2) ->
      ...
      if well_typed then (TE_arrow (te1, te2), ty2)
...
and type_args env loc params_ty el =
  ...
  let actual_types =
    List.rev
      (List.fold_left (fun res te -> List.rev_append te.texpr_type res) [] tel)
  in
  let well_typed = compatible actual_types params_ty in
  if well_typed then tel else error loc (ExpectedType (actual_types, params_ty))

and expected_type env e tt =
  let te = type_expr env e in
  let ty = te.texpr_type in
  if ty = tt then te else error e.pexpr_loc (ExpectedType (ty, tt))
...
let type_equation env eq =
  ...
  if well_typed then { teq_patt = patt; teq_expr = expr }
  else
    error eq.peq_expr.pexpr_loc
      (ExpectedType (expr.texpr_type, patt.tpatt_type))
...
let type_node n =
  ...
  let input =
    List.map
      (fun (x, ty) ->
        let x', _, _ = Gamma.find n.pn_loc env x in
        (x', ty))
      n.pn_input
  in
  let output =
