...
let declare_symbol name t_in t_out =
  let x = Hstring.make name in (* création d'un symbole *)
  Symbol.declare x t_in t_out; (* déclaration de son type *)
  x
...
let def_cpt n =
  ...
  let ite1 = (* ite(n = 0, 0, cpt(n-1)) *)
    Term.make_ite
      ...
      zero
      (Term.make_app cpt
         [ Term.make_arith Term.Minus n one ])
  in
...
let delta_incr n =
  Formula.make Formula.And [ def_cpt n ; def_ok n ; def_aux n ; ]
...
let base =
  ...
  BMC_solver.check();
  BMC_solver.entails ~id:0 (Formula.make Formula.And [p_incr zero; p_incr one])
